...
def conversation_start(
    ...
    print_header("Guided Conversation", f"Pattern: {pattern}\nMode: {mode}\nMax Attempts: {max_attempts}")
    ...
    if not script_path:
        print_error("Conversation script not found", f"Expected: scripts/{script_name}")
        raise typer.Abort()
    ...
    try:
        result = subprocess.run(cmd_args, cwd=kb_path, env={"GEMINI_API_KEY": api_key, **os.environ})
