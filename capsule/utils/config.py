...
class Config:
    ...
    def init_interactive(self) -> None:
        ...
        try:
            ...
            # API Key
            api_key = questionary.password("Enter your Gemini API key:", default=self.get("api.gemini_key", "")).ask()
            if api_key:
                self.set("api.gemini_key", api_key)

            # Knowledge Base Path
            kb_path = questionary.path("Path to TCM Knowledge Base:", default=self.get("paths.knowledge_base")).ask()
            if kb_path:
                self.set("paths.knowledge_base", kb_path)

            # Class ID
            class_id = questionary.text("Default class ID:", default=self.get("defaults.class_id")).ask()
            if class_id:
