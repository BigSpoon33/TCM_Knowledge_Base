...
<!--SR:!2025-11-10,3,250-->"""
    ...
    def generate_flashcards_for_file(self, formula_info: Dict, formula_category: str):
        ...
        if category:
            ...
            if card: self.formula_categories[formula_category]['category'].append(card)

        # Patterns
        patterns = '\n- '.join(fm.get('patterns', []))
        if patterns:
            card = self._format_flashcard(header, "What **TCM Patterns** does this formula treat?", f"- {patterns}")
            if card: self.formula_categories[formula_category]['patterns'].append(card)
        ...
        if symptoms:
            ...
            if card: self.formula_categories[formula_category]['symptoms'].append(card)
            
        # Diseases and Western Conditions
        diseases = '\n- '.join(fm.get('diseases', []))
        western = '\n- '.join(fm.get('western_conditions', []))
        disease_answer = []
        if diseases: disease_answer.append(f"**TCM Diseases:**\n- {diseases}")
        if western: disease_answer.append(f"**Western Conditions:**\n- {western}")
        if disease_answer:
            card = self._format_flashcard(header, "What **Diseases and Western Conditions** is this formula used for?", '\n\n'.join(disease_answer))
            if card: self.formula_categories[formula_category]['diseases_conditions'].append(card)
        ...
        for title, content in body_sections.items():
            ...
            question_title = re.sub(r'^\s*🌿\s*|\s*📝\s*|\s*🎯\s*|\s*🔄\s*|\s*⚠️\s*|\s*🔗\s*|\s*💡\s*|\s*📚\s*|\s*🔬\s*', '', title).strip()
    ...
    def generate_all(self):
        ...
        formula_files = sorted([f for f in self.formulas_dir.glob("*.md") if not f.name.endswith('.backup.md') and not f.name.startswith('TEMPLATE')])
        ...
        for filepath in formula_files:
            ...
            if not formula_info['frontmatter'].get('name'):
                ...
                if not h1_match: # Check the raw body if sections parsing failed
                     raw_body = open(filepath, 'r', encoding='utf-8').read().split('---', 2)[2]
    ...
    def save_flashcards_to_file(self):
        ...
        for formula_cat, card_types in sorted(self.formula_categories.items()):
            ...
            clean_cat = formula_cat.replace(' ', '_').replace('/', '_').replace('(', '').replace(')', '').replace('-', '_')
