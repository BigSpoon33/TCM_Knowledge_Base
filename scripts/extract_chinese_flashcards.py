...
class ChineseCharacterExtractor:
    ...
    def _extract_point_data(self, point_data: Dict, file_path: Path) -> None:
        ...
        if hanzi:
            self._add_term(hanzi, pinyin, english, "acupuncture_point", 
                          str(file_path), code)
    ...
    def _extract_herb_data(self, herb_data: Dict, file_path: Path) -> None:
        ...
        if hanzi:
            self._add_term(hanzi, pinyin, english, "herb", 
                          str(file_path), pharmaceutical)
    ...
    def _add_term(self, hanzi: str, pinyin: str, english: str, 
                  category: str, source_file: str, additional_info: str) -> None:
...
class StrokeOrderProvider:
    ...
    def get_hanzi_writer_svg(self, char: str) -> Optional[str]:
        ...
        if self.has_cjkradlib and self.decomposer:
            try:
                ...
                if sub_components:
                    ...
                    radical_info["components"] = [c for c in sub_components if ord(c) < 0x3000 or (0x4e00 <= ord(c) <= 0x9fff)]
...
class FlashcardGenerator:
    ...
    def generate_anki_csv(self, terms: Dict[str, Dict], 
                         character_analyses: Dict[str, Dict],
                         output_file: str = "tcm_chinese_flashcards.csv") -> None:
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            ...
            writer.writerow([
                ...
                "Character Breakdown", "Meanings & Radicals", "Stroke Order Links", "Tags"
            ])
            ...
            for hanzi, term_data in sorted(terms.items()):
                ...
                writer.writerow([
                    hanzi, pinyin, english, category,
                    breakdown_str, meanings_radicals_str, stroke_links_html, tags
                ])
        
        print(f"✓ Anki CSV generated: {output_path}")
    
    def generate_json_database(self, terms: Dict[str, Dict],
                               character_analyses: Dict[str, Dict],
                               output_file: str = "tcm_chinese_database.json") -> None:
    ...
    def generate_markdown_flashcards(self, terms: Dict[str, Dict],
                                     character_analyses: Dict[str, Dict],
                                     output_file: str = "tcm_chinese_flashcards.md") -> None:
    ...
    def generate_character_list(self, character_analyses: Dict[str, Dict],
                               output_file: str = "tcm_character_list.txt") -> None:
    ...
    def generate_obsidian_flashcards(self, terms: Dict[str, Dict],
                                     character_analyses: Dict[str, Dict],
                                     output_dir: str = "Flashcards/Chinese_Characters") -> None:
        ...
        for hanzi, term_data in terms.items():
            ...
            if category == "herb" and sources:
                ...
                if additional_info and "-" in additional_info:
                    ...
                    channel_map = {
                        "BL": "Bladder", "B": "Bladder",
                        "ST": "Stomach", "S": "Stomach",
                        "SP": "Spleen",
                        "HT": "Heart", "H": "Heart",
                        "SI": "Small Intestine",
                        "KI": "Kidney", "K": "Kidney",
                        "PC": "Pericardium", "P": "Pericardium",
                        "SJ": "San Jiao", "TH": "San Jiao", "TE": "San Jiao",
                        "GB": "Gall Bladder", "G": "Gall Bladder",
                        "LV": "Liver", "LIV": "Liver",
                        "LU": "Lung", "L": "Lung",
                        "LI": "Large Intestine",
                        "REN": "Ren", "CV": "Ren",
    ...
    def _generate_all_characters_flashcard(self, character_analyses: Dict[str, Dict],
                                          output_dir: Path, today: str) -> None:
    ...
    def _generate_category_flashcard(self, category: str, items: List[Tuple[str, Dict]],
                                     character_analyses: Dict[str, Dict],
                                     output_dir: Path, today: str) -> None:
    ...
    def _generate_herb_category_flashcard(self, herb_category: str, items: List[Tuple[str, Dict]],
                                         character_analyses: Dict[str, Dict],
                                         output_dir: Path, today: str) -> None:
    ...
    def _generate_point_channel_flashcard(self, channel: str, items: List[Tuple[str, Dict]],
                                         character_analyses: Dict[str, Dict],
                                         output_dir: Path, today: str) -> None:
    ...
    def _write_character_card(self, f, char: str, char_info: Dict) -> None:
        ...
        f.write(f"## What is the meaning, radical, and components of this character?\n")
        f.write("?\n")
    ...
    def _write_term_card(self, f, hanzi: str, term_data: Dict, character_analyses: Dict[str, Dict]) -> None:
        ...
        if english:
            f.write(f" · *{english}*")
        f.write("\n")
        f.write(f"## What are the individual characters and their meanings in this term?\n")
        f.write("?\n")
