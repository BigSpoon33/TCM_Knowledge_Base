...
def extract_herb(pdf, herb_name, start_page, end_page, herbs_dir):
    ...
    temperature = ""
    if properties:
        if 'acrid' in properties: taste.append('acrid')
        if 'bitter' in properties: taste.append('bitter')
        if 'sweet' in properties: taste.append('sweet')
        if 'sour' in properties: taste.append('sour')
        if 'salty' in properties: taste.append('salty')
        if 'bland' in properties: taste.append('bland')
        
        if 'warm' in properties: temperature = 'warm'
        elif 'hot' in properties: temperature = 'hot'
        elif 'cool' in properties: temperature = 'cool'
        elif 'cold' in properties: temperature = 'cold'
        elif 'neutral' in properties: temperature = 'neutral'
...
def main():
    ...
    for i, herb_name in enumerate(missing_herbs, 1):
        ...
        if herb_data:
            ...
            try:
                filepath = extract_herb(pdf, herb_name, herb_data['start'], 
                                      herb_data['end'], herbs_dir)
