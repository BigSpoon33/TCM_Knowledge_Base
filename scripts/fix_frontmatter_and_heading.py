...
def convert_frontmatter(old_frontmatter, filename):
    ...
    if 'western_conditions' in data and data['western_conditions']:
        ...
        wcs = data['western_conditions'] if isinstance(data['western_conditions'], list) else [data['western_conditions']]
        for wc in wcs:
            if wc:
                new_fm.append(f'- {wc}')
    else:
    ...
    if 'pattern_data' in data and isinstance(data['pattern_data'], dict):
        ...
        for key in ['pattern_type', 'pattern_subtype', 'excess_deficiency', 'hot_cold', 'interior_exterior', 'yin_yang']:
            if key in pd:
...
def fix_file(filepath):
    ...
    try:
        ...
        if "'[[" in old_frontmatter and "pattern-202" in old_frontmatter:
            print(f"  ✓ Already in new format, checking heading...")
            # Just fix heading
            ...
            for i, line in enumerate(lines):
                if line.strip().startswith('# ') and not line.strip().startswith('## '):
                    ...
                    if '=.' in line or '=' in line:
                        ...
                        print(f"  ✓ Fixed heading")
                        return True
                    elif '`=this.name`' in line:
                        print(f"  ✓ Heading already correct")
                        return False
                    break
            return False
        ...
        print(f"  ✓ Fixed frontmatter and heading")
        return True
