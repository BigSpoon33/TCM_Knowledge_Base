...
class MaterialsPackageGenerator:
    ...
    def __init__(
        self, root_note_path: Path, class_id: str = None, week: int = None, output_dir: Path = None, api_key: str = None
    ):
        ...
        self.results = {"flashcards": None, "quiz": None, "slides": None, "study_material": None, "tasks": None}
    ...
    def generate_all(self, **skips):
        ...
        if not skips.get("skip_flashcards"):
            self._generate_flashcards()
        if not skips.get("skip_quiz"):
            self._generate_quiz()
        if not skips.get("skip_slides"):
            self._generate_slides()
        if not skips.get("skip_conversation"):
            self._generate_guided_conversation()
        if not skips.get("skip_study"):
            self._generate_study_material()
        if not skips.get("skip_tasks"):
    ...
    def _generate_flashcards(self):
        ...
        try:
            ...
            flashcards = ai_gen.generate_from_content(content=self.parser.body, topic=self.metadata["name"])
    ...
    def _generate_quiz(self):
        ...
        try:
            ...
            questions = ai_gen.generate_from_content(content=self.parser.body, topic=self.metadata["name"])
            topic_slug = self.metadata["name"].replace(" ", "_")
        ...
        try:
            ...
            if is_pattern:
                slides_content = ai_gen.generate_pattern_slides(
                    content=self.parser.body, pattern_name=self.metadata["name"]
                )
            else:
                slides_content = ai_gen.generate_from_content(content=self.parser.body, topic=self.metadata["name"])
