...
class MaciociaDiagnosisExtractor:
    ...
    def _build_toc(self) -> List[Chapter]:
        ...
        toc.append(Chapter(1, "Observation of Body Shape, Physique and Demeanour", 
                          "Part 1: Diagnosis by Observation", 
                          "Section 1: Observation of Body, Mind and Complexion",
                          0, 0, 3))  # Pages TBD
        
        toc.append(Chapter(2, "Observation of the Mind, Spirit and Emotions",
                          "Part 1: Diagnosis by Observation",
                          "Section 1: Observation of Body, Mind and Complexion",
                          0, 0, 3))
        
        toc.append(Chapter(3, "Observation of the Complexion Colour",
                          "Part 1: Diagnosis by Observation",
                          "Section 1: Observation of Body, Mind and Complexion",
                          0, 0, 3))
        
        toc.append(Chapter(4, "Observation of Body Movements",
                          "Part 1: Diagnosis by Observation",
                          "Section 1: Observation of Body, Mind and Complexion",
                          0, 0, 3))
        ...
        for i, part in enumerate(body_parts, start=5):
            toc.append(Chapter(i, f"Observation of the {part}",
                              "Part 1: Diagnosis by Observation",
                              "Section 2: Observation of Parts of the Body",
                              0, 0, 3))
        ...
        toc.append(Chapter(50, "Tongue Diagnosis - Introduction",
                          "Part 4: Diagnosis by Palpation",
                          "Section 1: Tongue Diagnosis",
                          0, 0, 3))
        
        # Section on Pulse Diagnosis
        toc.append(Chapter(60, "Pulse Diagnosis - Introduction",
                          "Part 4: Diagnosis by Palpation",
    ...
    def extract_images_from_pages(self, start_page: int, end_page: int, 
                                  chapter_name: str) -> List[str]:
...
Format as markdown."""
    ...
    def format_as_markdown(self, chapter: Chapter, text: str, 
                          images: List[str]) -> str:
    ...
    def extract_chapter(self, chapter: Chapter, 
                       extract_images: bool = True,
                       analyze_images: bool = False) -> Optional[Path]:
        ...
        if chapter.start_page == 0:
            print(f"  ⚠️  Could not find pages for chapter")
            return None
        ...
        images = []
        if extract_images:
            images = self.extract_images_from_pages(
                chapter.start_page, chapter.end_page, chapter.title
            )
    ...
    def extract_all_chapters(self, extract_images: bool = True,
    ...
    def extract_specific_section(self, section_name: str, 
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract chapters from Maciocia Diagnosis book"
    )
    parser.add_argument(
        "--pdf",
    ...
    parser.add_argument(
        "--section",
        help="Extract specific section (e.g., 'Tongue Diagnosis')"
    )
    parser.add_argument(
        "--start-page",
        type=int,
        help="Start page for specific section"
    )
    parser.add_argument(
        "--end-page",
        type=int,
        help="End page for specific section"
    )
    parser.add_argument(
        "--no-images",
        action="store_true",
    ...
    parser.add_argument(
        "--analyze-images",
        action="store_true",
        help="Analyze images with Gemini vision model"
    )
    
    args = parser.parse_args()
    
    # Create extractor
    extractor = MaciociaDiagnosisExtractor(
    ...
    if args.section and args.start_page and args.end_page:
        extractor.extract_specific_section(
            args.section,
            args.start_page,
            args.end_page
        )
