...
class AutoDiagnosisExtractor:
    ...
    def get_subsections(self, start_page: int, end_page: int, parent_level: int) -> list[tuple[str, int, int]]:
    ...
    def extract_section(self, section_keyword: str, extract_images: bool = True) -> Path:
        ...
        markdown = self.create_markdown(section_keyword, text, images, start_page, end_page, subsections)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Automatically extract diagnostic sections from Maciocia book")
    parser.add_argument(
        "--pdf",
    ...
    parser.add_argument("--section", help="Section keyword to extract (e.g., 'Tongue Diagnosis', 'Pulse Diagnosis')")
    ...
    # Create extractor
    extractor = AutoDiagnosisExtractor(pdf_path=Path(args.pdf), output_dir=Path(args.output))
    ...
    try:
        ...
        if args.all_diagnostic:
            sections = ["TONGUE DIAGNOSIS", "PULSE DIAGNOSIS", "THE ART OF INTERROGATION"]
