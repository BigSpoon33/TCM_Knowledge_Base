...
from generate_flashcards_from_root import BASE_DIR, MATERIALS_DIR, find_root_note
...
Examples:
  ...
  python generate_guided_conversation.py "Lung Yin Deficiency" --max-attempts 5
    ...
    parser.add_argument("pattern", help='Name of the TCM pattern (e.g., "Lung Yin Deficiency")')
    parser.add_argument(
        "--script-only", action="store_true", help="Generate conversation script markdown instead of interactive mode"
    )
    parser.add_argument(
        "--max-attempts", type=int, default=3, help="Maximum attempts per heading before moving on (default: 3)"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    # Initialize engine
    try:
        engine = GuidedConversationEngine(root_note_path, max_attempts=args.max_attempts)
