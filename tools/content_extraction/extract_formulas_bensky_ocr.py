...
def parse_formula_text(text, formula_name):
    ...
    pinyin_match = re.search(
        r"^([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s*(?:Decoction|Powder|Pill|Tang|San|Wan)", text, re.MULTILINE
    )
    if pinyin_match:
    ...
    actions_match = re.search(
        r"(?:ACTIONS?|THERAPEUTIC EFFECTS?)[:\s]+([^\n]+(?:\n(?![A-Z]{2,})[^\n]+)*)", text, re.IGNORECASE
    )
    if actions_match:
    ...
    indications_match = re.search(
        r"(?:INDICATIONS?|CLINICAL MANIFESTATIONS?)[:\s]+([^\n]+(?:\n(?![A-Z]{2,})[^\n]+)*)", text, re.IGNORECASE
    )
    ...
    contra_match = re.search(
        r"(?:CONTRAINDICATIONS?|CAUTIONS?)[:\s]+([^\n]+(?:\n(?![A-Z]{2,})[^\n]+)*)", text, re.IGNORECASE
    )
    if contra_match:
